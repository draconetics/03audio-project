...
const UploadAudio = (props) =>{
    ...
    const getSuperDuration = (file) => {
        return new Promise(function(resolve, reject) {   // return a promise
            let objectUrl = URL.createObjectURL(file);
            var audio = new Audio(objectUrl);                     // create audio wo/ src
    ...
    const uploadAudio = async()=>{
        ...
        let duration = await getSuperDuration(fileInput.current.files[0]).then(resp=>resp).catch(e=>-1)
        if(duration === -1){
